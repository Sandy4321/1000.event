 ...
 * @license     http://www.gnu.org/licenses/gpl-3.0.html (GPLv3)
 * @author     Alexander Klabukov <sasha@klabukov.ru>
 * @version    $Id$
 */
class Sas_Controller_Plugin_Language extends Zend_Controller_Plugin_Abstract {
	...
	public function __construct(array $options = array(), Zend_Translate $translate = null, Zend_Locale $locale = null, $skipRegistry = false) {
		...
		$this->addLocale = (isset($options['addLocale'])) ? $options['addLocale'] : null;
	}
	...
	public function routeShutdown(Zend_Controller_Request_Abstract $request)
	{
		...
		if (is_array($this->addLocale)) {
			for($i=0;$i<count($this->addLocale);$i++) {
				$this->_translate->addTranslation($this->addLocale[$i]['path'], $this->addLocale[$i]['locale']);
			}
		}

		$language = $this->getRequest()->getParam($this->_lang_key, LANG_DEFAULT);

		if (!$this->_translate->isAvailable($language)) {
			throw new Zend_Controller_Action_Exception('This page dont exist (LANG).',404);
		}
...
//	public function addTranslation($translate) {
//		$this->_translate->addTranslation($translate);
//	}
